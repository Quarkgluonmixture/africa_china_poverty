"""
Training script for ResNet-18 model for crop yield prediction.
This script is based on the existing training infrastructure but uses our new ResNet-18 implementation.
"""
...
def run_training(sess, ooc, batcher_type, dataset, keep_frac, model_params, batch_size,
    ...
    - keep_frac: float
    - model_params: dict, parameters for ResNet-18 model
    - batch_size: int
    ...
    #        MODEL
    # ====================
    print('Building ResNet-18 model...', flush=True)
    model_params['num_outputs'] = 1

    with tf.variable_scope(tf.get_variable_scope()) as model_scope:
        train_model = ResNet18(train_batch['images'], is_training=True, **model_params)
    ...
    with tf.variable_scope(model_scope, reuse=True):
        train_eval_model = ResNet18(train_eval_batch['images'], is_training=False, **model_params)
        train_eval_preds = train_eval_model.outputs
        if model_params['num_outputs'] == 1:
            train_eval_preds = tf.reshape(train_eval_preds, shape=[-1], name='train_eval_preds')

    with tf.variable_scope(model_scope, reuse=True):
        val_model = ResNet18(val_batch['images'], is_training=False, **model_params)
...
def run_training_wrapper(**params):
    ...
    # create log and checkpoint directories if needed
...
if __name__ == '__main__':
    ...
    # paths
    flags.DEFINE_string('experiment_name', 'resnet18_experiment', 'name of experiment being run')
    ...
    # learning parameters
    flags.DEFINE_string('label_name', 'wealthpooled', 'name of label to use from TFRecord files')
    ...
    flags.DEFINE_float('lr_decay', 1.0, 'Decay rate of learning rate (default 1.0 for no decay)')
